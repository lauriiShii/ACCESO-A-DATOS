...
public class LibreriaJDBC {
  ...
  //SELECT PASANDO LA SENTENCIA
	...
	public static void dameHorarioCurso (String codOe, String codCurso, boolean desdoble) throws SQLException{
		int cont = 1;
		...
		System.out.println();
		if(desdoble)
			sentencia = (PreparedStatement) conexion.prepareStatement("select CodAsignatura from horario where codOe = ? and CodCurso = ? and CodAsignatura in "
		...
		while (result.next()){
			...
			if (cont <= 5){
				if (result.getString(1).matches("@.+"))
					System.out.printf("%6s%-9s* |", "", result.getString(1));
				else 
					System.out.printf("%6s%-9s  |", "", result.getString(1));
			}
			else {
				if (result.getString(1).matches("@.+"))
					System.out.printf("\n%6s%-9s* |", "", result.getString(1));
				else
					System.out.printf("\n%6s%-9s  |", "", result.getString(1));
				cont = 1;
			}
	...
	//DONDE ESTA UN PROFESOR
	public static void dondeEstaProfesor (String codProf, String codTramo) throws SQLException{
		
		ResultSet result;
		PreparedStatement sentencia = (PreparedStatement) conexion.prepareStatement("select codOe, codCurso from horario where codTramo = ? and codAsignatura in (select codAsignatura from reparto where codProf = ?);");
	...
	//EJECUTAR SCRIPT CREACION
    public static String ejecutarScriptCreacion() throws IOException {
